...
| A4 | **乾坤闘気弾を GCDブロック先頭（必殺技より前）へ移動**。`LevelChecked` と `!InMeleeRange()` 条件を削除 | 上流は GCDブロック内（必殺技の後）。`CanFiresReply()` に集約 | 🟡 実測後 |
| A5 | **絶空拳を同様に移動**。条件を `HasEffect(RiddleOfWind) && 残り<4秒` に変更（元は `!InMeleeRange() \|\| !HasEffect(PerfectBalance)`） | `CanWindsReply()` | 🟡 実測後 |
| A6 | 踏鳴ブロックの**後にも**乾坤/絶空拳の判定を追加（2箇所目） | 該当なし | 🟡 実測後 |
...
| # | 変更 | 上流最新 | 判断 |
| --- | --- | --- | --- |
| B5 | **乾坤/絶空拳を関数最上部（開幕判定より前）へ引き上げ**（3ブロック: 乾坤残2秒 / 絶空拳 / 乾坤遠距離） | GCDブロック内に据え置き | 🟡 実測後 |
| B6 | **桃園結義を紅蓮の極意より前へ移動** | **上流も同じ順序に変更済み** | ⚪ 不要 |
| B7 | oGCD順を 踏鳴 → 陰陽闘気斬 → 疾風 に変更（土台: 桃園 → 疾風 → 踏鳴 → ヒール → 陰陽闘気斬） | 踏鳴 → 疾風 → **陰陽闘気斬** の順 | 🟡 実測後 |
| B8 | 陰陽闘気（Meditation）判定を演武の後にも追加（計2箇所） | 1箇所のみ | 🟡 実測後 |

### B-3. 発動条件の変更

| # | 変更 | 上流最新 | 判断 |
| --- | --- | --- | --- |
| B9 | 陰陽闘気の条件を大幅変更（`IsOriginal(MasterfulBlitz)`・紅蓮/絶空/乾坤バフ条件を削除、`!HasEffect(Brotherhood)` を追加） | `CanMeditate()` に集約 | 🟡 実測後 |
| B10 | 演武（FormShift）を戦闘外限定から解放。非近接時も可、紅蓮残18秒以下、`isWindBh`/`readyBlitz`、Thunderclap 中は除外 | `CanFormshift()` | 🟡 実測後 |
...
| B14 | 桃園結義を踏鳴スタック数と連動（`GetBuffStacks(PerfectBalance) <= 1` 等）、`Gauge.BlitzTimeRemaining >= 2000` | `IsBrotherhoodInPerfectBalanceWindow()` が別解を実装 | 🟡 実測後 |
| B15 | 紅蓮の極意: `SubOption` 削除 → `MNK_ST_RiddleOfFire_HP` | `MNK_ST_RoFHPOption` + `RoFHPBossOption` | ⚪ 不要 |
| B16 | 紅蓮の極意を**桃園CDと同期**（残54〜66秒 or 114秒以上）、`RemainingGCD <= 1` | `CanRoF()`。同期概念は `IsEvenWindowApproaching()` 等が担当 | 🟡 実測後 |
| B17 | oGCDブロックの入口を `CanWeave()` → `CanWeave() \|\| !HasBattleTarget()`（ターゲット無しでも発動） | `CanWeave()` のまま | 🟡 実測後 |
| B18 | 陰陽闘気斬に「桃園直後は撃たない」条件（`!JustUsed(Brotherhood, 122)`） | `CanUseChakra()` | 🟡 実測後 |
| B19 | 疾風の極意: `SubOption` 削除 → HP%、距離10以内、`isPreserveMode` 除外 | `MNK_ST_RoWHPOption` + Boss判定、`CanRoW()` | ⚪ 不要（距離条件は要確認） |
| B20 | ComboHeals に `PlayerHealthPercentageHp() <= 99` ガード追加 | 無し | 🟡 実測後 |
| B21 | **金剛の返し（EarthsReply）を ST に追加**（残6秒未満） | `MNK_ST_EarthsReply` + `CanEarthsReply()` として実装済み | ⚪ 不要 |
| B22 | FormlessFist の GCD 判定に `canMelee` と Blitz 残り時間条件を追加 | `ForcedOpoGCD()` / `ForceSecondOpo()` | 🟡 実測後 |

### B-4. 必殺技・踏鳴・チャクラ

| # | 変更 | 上流最新 | 判断 |
| --- | --- | --- | --- |
| B23 | **必殺技の条件を全面書き換え** — `InMasterfulRange()` を廃し、Blitz残り時間・桃園CD・`Fast_Phoenix`・`ROFLastOnly` を絡めた複合条件に | `CanMasterfulBlitz()` / `ShouldSpendMasterfulBlitz()` | 🟡 実測後 |
...
| B25 | チャクラ数による分岐を新設（`>=2` で該当型、2種が1個ずつなら残り1種） | 同等ロジックあり | ⚪ 不要 |
...
| B29 | 乾坤/絶空拳の「通常位置」の条件も書き換え（`FiresReply_Order`、距離20/10、桃園・紅蓮の残り時間） | `CanFiresReply()` / `CanWindsReply()` | 🟡 実測後 |
...
| # | 変更 | 上流最新 | 判断 |
| --- | --- | --- | --- |
| D1 | `raptorAction` / `coeurlAction` / `maxPowerSkill` を変数化（レベルと近接可否でフォールバック） | 同等の集約あり | ⚪ 不要 |
| D2 | 乾坤/絶空拳を関数最上部に追加（ST の B5 と同思想） | GCDブロック内 | 🟡 実測後 |
| D3 | 陰陽闘気/演武の条件を `!InCombat()` のみに簡素化 | `CanMeditate(onAoE)` / `CanFormshift()` | 🟡 実測後 |
| D4 | **桃園結義を oGCDブロックの外（紅蓮より前）へ移動** | 上流も桃園→紅蓮の順 | ⚪ 不要 |
| D5 | 紅蓮の極意に桃園CD同期条件（54〜66秒 / 114秒以上）を追加 | 該当なし | 🟡 実測後 |
| D6 | **`UsePerfectBalanceAoE()` を新設**して oGCD 先頭へ | `MNK_AoEUsePerfectBalance` + `CanPerfectBalanceMaxChargeAoE()` として実装済み | ⚪ 不要 |
| D7 | 万象闘気圏を `OriginalHook(InspiritedMeditation)` → **`OriginalHook(Enlightenment)` に修正**（土台はチャクラ蓄積技を返していた＝上流のバグ） | 上流も修正済み | ⚪ 不要 |
| D8 | **金剛の極意 / 金剛の返しを追加**（`MNK_AoE_RiddleOfEarth_Threshold`、PT平均HPで判断） | `MNK_ST_UseRoE` / `CanEarthsReply()` が PT平均HPを使用 | ⚪ 不要 |
| D9 | FormlessFist の GCD 判定を追加 | あり | ⚪ 不要 |
| D10 | 必殺技の条件を書き換え（ST の B23 の簡略版） | `CanMasterfulBlitz(true)` | 🟡 実測後 |
| D11 | 踏鳴中のチャクラ選択を全面書き換え（ST の B24〜B25 と同思想） | `DoPerfectBalanceCombo(ref actionID, onAoE: true)` | 🟡 実測後 |
| D12 | Open Lunar に `ElixirField` 追加 + `compareNextBurstTime` | 該当なし | 🔴 要移植 |
| D13 | 通常ローテを `raptorAction` / `coeurlAction` に置換（LV判定の入れ子を解消） | 同等 | ⚪ 不要 |
...
| F7 | **`UsePerfectBalance()` を全面書き換え** — 奇数窓/偶数窓/低レベル/追加 の4分岐。`Fast_Phoenix`・`Many_PerfectBalance`・`ROFLastOnly`・`AdjustROF` 連動、ターゲット無し時の分岐追加 | `CanPerfectBalance()` + `ShouldUsePreRoFPerfectBalance()` 等に分解済み | 🟡 実測後（設定化するなら要移植） |
...
| G9 | `MNK_ST_Fast_Phoenix`（2択）新設 | 該当なし（固定ロジック） | 🟡 実測後 |
...
全 **82項目**（うち MNK.cs が 52、Helper 11、Config 12、Preset 5、その他 2）。

| 判断 | 件数 | 主な内容 |
| --- | --- | --- |
| 🔴 **要移植** | **29件** | 多ボタン設計（B1〜B4）、`compareNextBurstTime`（F5）、`Phoenix_Order`（B28）、7秒opener（F9/F10）、PT平均ヒール（A1/A2/C1/C2）、トゥルーノース制御（B30） |
| 🟡 **実測後** | 25件 | 上流が固定ロジックで別解を持つもの。素の上流を使って不満が出た項目だけ |
| ⚪ **不要** | 28件 | 上流が同等以上を実装済み、または移動のみ・API追従 |
| 🐛 **バグ** | 5件 | うち2件は挙動に実害あり |

セクション別の項目数: A(STシンプル) 7 / **B(STアドバンスト) 30** / C(AoEシンプル) 2 / D(AoEアドバンスト) 13 / E(その他コンボ) 2 / F(Helper) 11 / G(Config) 12 / H(Preset) 5
