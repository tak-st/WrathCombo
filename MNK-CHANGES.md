...
移植を実装し、ゲーム内で使いながらフォークの該当箇所と突き合わせた結果、
**当初の洗い出しに誤りが3件、漏れが3件、判定違いが1件**見つかった。原因はいずれも同じで、
**上流に同名・同趣旨の処理があると「同等」と早合点した**こと、および
**差分だけを見て、フォークが「動かしていない」箇所の意味を読まなかった**ことによる。

#### 誤り（フォークに無いものを「ある」と書いていた）

| # | 当初の記述 | 実際 |
| --- | --- | --- |
| B26 | Open Lunar に `compareNextBurstTime(0, 20)` を導入 | フォークの `compareNextBurstTime` は**トゥルーノース判定の1箇所のみ**。Open Lunar は `GetCooldownRemainingTime(Brotherhood) <= 20` だけで判断している |
| B27 | Open Lunar の `JustUsed` に `ElixirField` を追加 | フォークは `ElixirBurst` のみ。Lv80〜99 では第2項が成立しない状態だった |
| D12 | AoE Open Lunar にも上記2つ | どちらも無い |

**ナディ選択への影響**: 両ナディが無い状態では第1項 `!LunarNadi` が真になるため、
判断は Brotherhood の残リキャストのみに帰着する。
補正後のバースト窓（`compareNextBurstTime`）を `&&` で足すと条件が狭まる方向にしか効かず、
`ずらした場合、次の紅蓮に合わせる` が ON で桃園が紅蓮より10秒以上先行している場面で
**月を貯めるべき窓に日を貯め始める**。実装から除去した。

#### 漏れ（フォークにあるのに行が無かった）

| # | 内容 | 見落とした理由 |
| --- | --- | --- |
| B32 | 踏鳴中の Coeurl GCD をトゥルーノースに差し替え | 通常コンボ側（`DoBasicCombo`）には同等の処理が移植済みだったため、踏鳴側の欠落に気付かなかった |
| D14 | AoE アドバンストの内丹・ブラッドバスを PT平均差に | 単体アドバンストだけは土台のままだったため、「単体は変更なし＝この変更はシンプルモード限定」と誤読した。実際は**4箇所すべて**が変更されていた（単体アドバンストは後述の 2026-08-25 訂正2 を参照） |
| D15 | AoE Open Lunar の括弧が ST と異なる | 条件式を意味単位で読み、`&&` と `\|\|` の結合順を追っていなかった |

#### 判定違い

| # | 当初 | 訂正後 |
| --- | --- | --- |
| D4 | ⚪ 不要（「上流も桃園→紅蓮の順」） | 🔴 要移植。**本質は順序ではなく `CanWeave()` ゲートの外にあること**。フォークは桃園と紅蓮をウィービング判定なしで全 GCD より先に返しており、上流の構造に合わせるとウィービング枠が無い間 GCD に譲ってしまう |

#### 配置の追認

`B21`（金剛の返しの「消えかけたら吐く」）は、当初「トリガのみ移植、置き場所は上流の
`MNK_ST_UseRoE` 配下でよい」と判断した。しかし上流の置き場所は
`MNK_ST_UseRoE` + `MNK_ST_EarthsReply` の**2つのスイッチの下**であり、どちらかを切ると腐る。
フォークは `MNK_ST_ComboHeals` 配下に独立して置いていた。
**金剛の返しは支払い済みのリソースで、腐らせる以外に損のしようがない**ため、フォークの配置に合わせた。

#### 教訓

> **上流に同名の関数があっても、条件が同じとは限らない。**
> **フォークが「触っていない」箇所こそ、触った箇所の意味を決めている。**

`D14` は後者の典型で、単体アドバンストが土台のままに見えたことが
「この変更はシンプルモード限定」という誤った読みを生んだ。
**その「土台のまま」自体が、フォーク側のマージ事故だった**（2026-08-25 訂正2）。

---

### 2026-08-25 の実装後の訂正（バーストのズレ）

実戦ログ（木人8分45秒、桃園5回）を解析したところ、**桃園が毎サイクル 0.69〜0.77秒ずつ遅れ、5回目で累積2.915秒**ずれていた。紅蓮も同額（2.914秒）引きずられていた。奇数窓で単独使用される紅蓮は +0.01〜0.08秒 で出せていたので、ウィービング機構自体は正常。

原因は移植側の2点だった。

#### 誤り — 踏鳴の窓の基準を「桃園と紅蓮の遅い方」にしていた

`5fa0baac6`（ペアリングの設定化）で `BurstStartsIn()` を新設し、`ShouldUseEvenWindowPerfectBalance()` と `EvenWindowCannotWaitForOpo()` の基準を `GetCooldownRemainingTime(Brotherhood)` から差し替えた。設定が `2つ揃えて撃つ`（既定）だと `Math.Max(桃園, 紅蓮)` を返す。

紅蓮は60秒リキャストで奇数窓に単独使用されるため、偶数窓では桃園より 0.7〜1.5秒 後ろになる。その分だけ窓の距離が水増しされ、2GCDの上限を超えて却下されていた。

| バースト | 桃園残 | 紅蓮残 | 上限 | フォーク基準 | 移植基準 |
| --- | --- | --- | --- | --- | --- |
| 2分 | 5.096 | 5.787 | 5.177 | ○ | ✗ |
| 4分 | 5.086 | 6.536 | 5.177 | ○ | ✗ |
| 6分 | 5.064 | 5.744 | 5.177 | ○ | ✗ |
| 8分 | 5.075 | 5.773 | 5.177 | ○ | ✗ |

**フォークは一貫して桃園のリキャスト単体で測っている。** 踏鳴の窓は「必殺技を桃園のバフに乗せる」ためのもので、紅蓮を待つ理由がない。`BurstStartsIn()` は削除し、`MNK_ST_Burst_Pairing` は `CanBrotherhood()` の紅蓮待ちを制御する本来の役割だけにした。

#### 漏れ — バーストの先読み（B33）

フォークは桃園を**リキャスト明けの0.7秒前**から、紅蓮を **`GCD - 0.99` ≒ 0.96秒前**からボタンに出していた。移植版は `ActionReady()` のみ。

リキャスト明けがGCDの上がりより後ろに来る位相では、その瞬間ボタンがGCDのままになり、押下でGCDが出てバーストが1枠まるごと遅れる。**バーストは次回の開始が今回の使用時刻から始まるので、この遅れは毎サイクル同額で積み上がり、二度と戻らない。**

#### 新規実装 — 窓の判定を秒からGCD数へ

上記2点を直しても、フォークと同じ「2GCD前まで」という**時間の上限**では「2GCD前の猿」と「直前の猿」を区別できない。後者を採ると窓が開いた瞬間にバーストが来て、踏鳴を1枚も消費できない。

そこで判定を「この枠で窓を開いたら、桃園が明けるまでに何枚消費できるか」に置き換えた。

```csharp
private static int PerfectBalanceGcdsBeforeBurst()
{
    float untilBurst = GetCooldownRemainingTime(Brotherhood);
    if (untilBurst < RemainingGCD)
        return 0;
    return (int)((untilBurst - RemainingGCD) / GCD) + 1;
}

// 1枚（疾風の返しを収める窓なら2枚）以上、かつ3枚以下
int spentByBurst = PerfectBalanceGcdsBeforeBurst();
return spentByBurst >= (WindowWillHaveWindsReply() ? 2 : 1) && spentByBurst <= 3 && ...
```

猿のGCDは3回に1回なので、隣り合う猿の消費見込みは3ずつ飛び、位相によって `{0,3,6}` `{1,4,7}` `{2,5,8}` のいずれかになる。

- 必要1枚 … どの位相でも猿だけで足りる
- 必要2枚 … `{1,4,7}` の位相だけ猿では届かず、猛虎（±1）が要る

**移植時に入れた「`WindowWillHaveWindsReply()` のときだけ猛虎の直後も許す」という条件は、猛虎が必要になる場面と正確に一致していた**（作者確認済み）。当初これをフォークからの退行と判断したのは、時間の上限のまま見ていたための誤り。フォークが猛虎を無条件に許していたのは、時間の上限では猿だけでは足りない位相があるためで、GCD数で測るなら条件付きで十分。

#### 結果

20分（桃園10サイクル）の実測で**累積ずれ1秒未満**。修正前の 0.73秒/サイクル なら同じ20分で約7.3秒ずれる計算だった。

#### 教訓

> **周期の等しいリソースを「遅い方に合わせる」と、遅れが毎周期積み上がる。**
> **時間の上限は「近すぎる」を弾けない。数えるべきものは数える。**

---

### 2026-08-25 の訂正2（土台ブランチの選定ミス）

**この洗い出しは `origin/main` を「フォークの最終形」として作成したが、`main` は上流 1.0.0.14 をマージした後の状態であり、そのマージで独自改造が1箇所巻き戻っていた。**

```
noPotion (2025-02-26)  ← 内丹・ブラッドバスは4箇所すべて PT平均差
      ↓
main (2025-03-28) = noPotion + Merge tag '1.0.0.14' + fix 3件
                     ↑ このマージで単体アドバンストの2行が上流の絶対値判定に戻った
```
...
`main` のスライダーのラベルが `"PT平均から自身のHP%が乖離したら {0} を使用 (100で無効)"` なのに、実装は `Role.CanSecondWind()` ＝ 絶対値だった。当初これを「フォーク時点のラベルと実装の食い違い」と判断したが、作者本人が「PT差分で動いていた記憶がある」と指摘。**ラベルの方が正しく、実装が後から巻き戻されていた。**

#### 影響範囲の確認

`main` が他にも独自改造を失っていないか、フォーク特有の構文12種を両ブランチで数えた。

| 構文 | noPotion | main | |
| --- | --- | --- | --- |
| `GetPartyAvgHPPercent() - PlayerHealthPercentageHp` | 8 | **6** | ← 唯一の減少 |
| `positionCheck` | 0 | 3 | マージ後に追加 |
| `isPreserveMode` | 0 | 16 | 同上 |
| `Phoenix_Order` | 8 | 10 | 同上 |
| `Brotherhood, 124` | 1 | 2 | 同上 |
| `<= 0.7` | 2 | 3 | 同上 |
| `compareNextBurstTime` / `compareCooldownTime` / `maxPowerSkill` / `17.35` / `Many_PerfectBalance` / `GCD - 0.99` | 同数 | 同数 | 維持 |

**減少したのは PT平均差の2件のみ**で、それが今回の単体アドバンストの2行。他はすべて同数または `main` の方が多い（`positionCheck` `isPreserveMode` `Phoenix_Order` はマージ後に追加された機能）。

したがって **`main` は他のすべてにおいて `noPotion` 以上であり、土台としては `main` のままで正しい**。今回の1箇所だけが例外。

#### 既定値

フォークは4箇所すべて **内丹 25 / ブラッドバス 40** で統一されており、シンプルモードのハードコード値とも一致する。

| 設定 | フォーク | 上流（移植時に引き継いでいた値） |
| --- | --- | --- |
| `MNK_ST_SecondWindThreshold` | **25** | 40 |
| `MNK_ST_BloodbathThreshold` | **40** | 30 |
| `MNK_AoE_SecondWindThreshold` | **25** | 40 |
| `MNK_AoE_BloodbathThreshold` | **40** | 30 |
| `MNK_AoE_RiddleOfEarth_Threshold` | 90 | 90 |

上流の 40 / 30 は差分判定としては前後が逆（回復のブラッドバスより内丹が後に出る）。4箇所とも 25 / 40 に統一した。

#### 教訓

> **フォークのラベルと実装が食い違っていたら、まず「マージで実装だけ巻き戻った」を疑う。**
> **土台に選んだブランチが本当に最終形か、祖先ブランチと突き合わせて確認する。**

差分だけを見ていると、巻き戻った箇所は「元から土台と同じ」＝独自改造なしにしか見えない。
`D14` の見落としも、この巻き戻りが「単体アドバンストは変更されていない」という誤った前提を作ったことが遠因だった。
...
| B21 | **金剛の返し（EarthsReply）を ST に追加**（残6秒未満＝**消えかけたら吐く**）。プリセットは `MNK_ST_ComboHeals` 配下 | ST に存在はするが `CanEarthsReply()` は **PT平均HP ≤ 閾値 かつ PT の75%が範囲内**。**健康なら腐らせる**。置き場所は `MNK_ST_UseRoE` + `MNK_ST_EarthsReply` | 🔴 要移植（トリガのみ。ST への追加自体は不要）。**配置もフォーク側に合わせる** — 下記「実装後の訂正」 |
...
| B26 | Open Lunar 条件に `Many_PerfectBalance` を導入 | 該当なし | 🔴 要移植（**2026-08-21 訂正**: `compareNextBurstTime(0, 20)` は Open Lunar に無い。下記「実装後の訂正」） |
| B27 | ~~Open Lunar の `JustUsed` 判定に `ElixirField` を追加~~ | — | ⚪ 不要（**2026-08-21 訂正**: フォークも `ElixirBurst` のみ。追加は移植側の判断） |
...
| B34 | **単体アドバンストの内丹・ブラッドバスも PT平均差**（`noPotion` 時点。`main` はマージで巻き戻り）。ブラッドバスには `HasBattleTarget()` も付く | `Role.CanSecondWind(閾値)` ＝自分のHPの絶対値 | 🔴 要移植（**2026-08-25 追加**。`A1`/`A2`/`C1`/`C2`/`D14` と同じ変更の4箇所目） |
| B33 | **バーストの先読み** — 桃園は `IsOffCooldown(Brotherhood) \|\| GetCooldownRemainingTime(Brotherhood) <= 0.7`、紅蓮は `\|\| <= (GCD - 0.99)` でリキャスト明け前からボタンに出す | `ActionReady()` のみ。**明けがGCDの上がりより後ろに来た位相で、丸ごと1ウィービング枠を失う** | 🔴 要移植（**2026-08-25 追加**） |
| B32 | **踏鳴中の Coeurl GCD をトゥルーノースに差し替え**（`!positionCheck(actionID) && useTrueNorth ? TrueNorth : …`）。`useTrueNorth = MNK_STUseTrueNorth && !isPreserveMode` | `CoeurlFormGCD()` は型のみを返す。上流は通常コンボ側でしか方向指定を補わない | 🔴 要移植（**2026-08-21 追加**。当初の洗い出しから漏れていた） |
...
| D4 | **桃園結義・紅蓮の極意を oGCDブロックの外へ移動**（ST も同じ） | 上流は `CanWeave()` ゲートの**内側**。順序は桃園→紅蓮で同じだが、**ウィービング枠が無いと GCD に譲る** | 🔴 要移植（**2026-08-21 訂正**: ⚪ 不要 から変更。本質は順序ではなく「ゲートの外」） |
...
| D12 | ~~Open Lunar に `ElixirField` 追加 + `compareNextBurstTime`~~ | — | ⚪ 不要（**2026-08-21 訂正**: フォークの AoE Open Lunar はどちらも持たない） |
| D13 | 通常ローテを `raptorAction` / `coeurlAction` に置換 | 単なる置換ではなく **Fury 判定と近接フォールバックが入る**。D1 と一体 | 🔴 要移植 |
| D14 | **内丹・ブラッドバスを PT平均差に**（`(PT平均HP% − 自分HP%) >= 設定値`） | `Role.CanSecondWind(閾値)` のまま＝自分HPの絶対値 | 🔴 要移植（**2026-08-21 追加**。`A1`/`A2`/`C1`/`C2` と同じ変更の4箇所目。当初 D セクションに行が無かった） |
| D15 | Open Lunar の括弧が ST と異なり、`HasEffect(Brotherhood)` が**独立項**になっている（`((!LunarNadi \|\| …) && BH<=20) \|\| HasEffect(BH)`） | 該当なし | 🔴 要移植（**2026-08-21 追加**。桃園発動中は月ナディが開いていても月を貯め直す） |
...
全 **88項目**（うち MNK.cs が 57、Helper 11、Config 12、Preset 5、その他 2）。

2026-08-21 に 3件追加（`B32` `D14` `D15`）、`D4` を ⚪ から 🔴 へ、`B27` `D12` を 🔴 から ⚪ へ変更した。
2026-08-25 に `B33`（バーストの先読み）と `B34`（単体アドバンストのPT平均ヒール）を追加した。上記「実装後の訂正」3節を参照。

| 判断 | 件数 | 主な内容 |
| --- | --- | --- |
| 🔴 **要移植** | **70件** | 多ボタン設計（B1〜B4）、`compareNextBurstTime`（F5）、**桃園と踏鳴の連動（B14）**、**踏鳴のタイミング制御（F7 / F-2）**、`Phoenix_Order`（B28）、7秒opener（F9/F10）、PT平均ヒール（A1/A2/C1/C2/D14/B34）、トゥルーノース制御（B30） |
| 🟡 **実測後** | 0件 | 上流が固定ロジックで別解を持つもの。素の上流を使って不満が出た項目だけ |
| ⚪ **不要** | 18件 | 上流が同等以上を実装済み、または移動のみ・API追従 |
| 🐛 **バグ** | 5件 | うち2件は挙動に実害あり |

セクション別の項目数: A(STシンプル) 7 / **B(STアドバンスト) 34** / C(AoEシンプル) 2 / D(AoEアドバンスト) 15 / E(その他コンボ) 2 / F(Helper) 11 / G(Config) 12 / H(Preset) 5
...
4. **PT平均HPによるヒール（A1, A2, C1, C2, D14, B34）** — 4つのローテすべて。同じ1行の置換で済む、最も安上がりな移植
